...
const setupOwnerUser = async () => {
    setupApiUsers()
    useAuth.mockReturnValue({ user: { username: mockIngrediantListAuthor } })
...
describe("As all users", () => {
    ...
    test.each(testCases)(
        "As: %s: If I uncheck an item it stays at the bottom of the list",
        async (label, setupFn = () => {}) => {
            const { user } = await setupFn()
            expect(getListItems()).toHaveLength(3)
            await user.click(screen.getAllByRole("checkbox")[0])
...
describe("As users with edit privileges", () => {
    ...
    test.each(testCases)(
        "As: %s: If I enter no ingrediant name I see an error message",
        async (label, setupFn = () => {}) => {
            const { user } = await setupFn()
...
describe("As users that get the list from the API", () => {
    ...
    test.each(testCases)(
        "As: %s: If the list does not exist, I see a Not Found message",
        async (label, setupFn = () => {}) => {
    ...
    test.each(testCases)(
        "As: %s: The name of the list is displayed (if there is one)",
        async (label, setupFn = () => {}) => {
            apiIngrediantList.getSingle.mockResolvedValue(mockIngrediantList)
            setup(<ListMaker />)
    ...
    test.each(testCases)(
        "As: %s: If there is no list name, then I see 'Unnamed List'",
        async (label, setupFn = () => {}) => {
            mockIngrediantList.name = ""
            apiIngrediantList.getSingle.mockResolvedValue(mockIngrediantList)
            setup(<ListMaker />)
...
describe("As a not logged in user viewing the index page", () => {
    test("If I reload the page the information I have entered has been saved", async () => {
        ...
        unmount(<ListMaker useLocalView />)
